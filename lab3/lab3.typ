...
a. Условные переходы (Branching / Conditional Jumps):
- ```asm JE``` (Jump if Equal) - Переход, если равно.
- ```asm JNE``` (Jump if Not Equal) - Переход, если не равно.
- ```asm JZ``` (Jump if Zero) - Переход, если ноль (аналогично ```asm JE```).
- ```asm JNZ``` (Jump if Not Zero) - Переход, если не ноль (аналогично ```asm JNE```).
- ```asm JG``` (Jump if Greater) - Переход, если больше.
- ```asm JGE``` (Jump if Greater or Equal) - Переход, если больше или равно.
- ```asm JL``` (Jump if Less) - Переход, если меньше.
- ```asm JLE``` (Jump if Less or Equal) - Переход, если меньше или равно.

b. Безусловные переходы (Unconditional Jumps):
- ```asm JMP``` (Jump) - Безусловный переход.
c. Циклы (Loops):
- ```asm LOOP``` – Уменьшает счетчик цикла (```asm CX```), и если он не равен нулю, выполняет переход.

2. *Выделите в своей программе фрагмент, реализующий ветвление. Каково назначение каждой машинной команды фрагмента?*

Сравниваем ```asm ecx``` и `0`: 
#code("asm cmp ecx, 0
jl  f1 ; Условный переход, если ecx < 0 (результат отрицателен)
jmp f2 ; Иначе", "asm", "Ветвление")

3. *Чем вызвана необходимость использования команд безусловной передачи управления?*
- ```asm JMP``` используется для эффективного перехода между различными участками кода, такими как циклы или ветвления.
- может быть применен для перехода к обработчику ошибок при возникновении исключительных ситуаций.
- используется для возврата из вызванной функции обратно в вызывающую функцию.
