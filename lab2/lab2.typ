...
После ввода числа пользователь нажимает на enter (LF) и число записывается по одной цифре в память. Теперь нам необходимо достать каждую цифру, вычесть из нее код числа $0$ (```asm sub eax, '0'```) и сложить эту цифру с предыдущими, умноженными на 10.

Для этого напишем новую процедуру, которая будет брать адрес строки в ```asm edx``` и длину этой строки в ```asm ecx``` и после преобразования этой строки в число класть это числа регистр ```asm eax```.
...
В стек загружаем все использованные в процедуре регистры, чтобы их не потерять после окончания выполнения. Готовим регистры для цикла ```asm eax```, как регистр для арифметических операций, будет содержать в себе все число, что мы получили, поэтому перемещаем в него 0. Так как мы будем производить умножение, чтобы не потерять адрес, куда в последствии необходимо сохранить число переместим его в регистр адреса ```asm esi```.
...
Но перед циклом необходимо написать обработчик одной цифры. Цифра эта будет занимать 1 байт, поэтому процедура будет брать только регистр ```asm dl``` и работать дальше с ним.
...
Тут нам длина буфера уже не важна, ведь он уже прочитан. Поэтому заносим в регистр ```asm ecx``` флаг минуса, который лежит наверху стека. Теперь когда у нас есть модуль числа можно и запросить его дополнительный код, если это необходимо. Проверяем ```asm ecx``` и если там лежит 1 переходим к метке для умножения ```asm eax``` на $-1$, иначе в конец.
...
Опять убираем все используемые регистры в стек.
...
Добавляем минус в буфер, прибавляем единицу в адрес, что бы этот минус не затерся последней цифрой числа (ведь при делении на 10 мы первой получим именно последнюю цифру). После этого берем само число по модулю, т.е. преобразуем его дополнительный код в нормальный. Потом так же переходим в цикл.
...
И переходим снова на метку цикла, чтобы начать новую итерацию.
...
Для этого просто перемещаем приглашение пользователю, длину приглашения, ссылку на буфер для данных и длину буфера в нужные регистры и вызываем ```asm geti```. Проделываем эту процедуру 3 раза и получаем в памяти 3 числа, готовых к обработке.

Все выполнения арифметических операций будет в```asm calc``` переходим в нее, когда значения успешно введены.
...
Ввод, вывод и код лабораторной я разнес в отдельные файлы, которые и назвал ```sh input.asm```,
...
#grid(
  ...
  img(image("img/4.png", width: 90%),"Обрабатываем минус")
)
#grid(
  columns:2,
  gutter:10pt,
...
Для проверки ```asm calc``` необходимы числа поменьше, поэтому перезапустим программу и с помощью команды ```sh break calc``` установим точку останова уже непосредственно перед арифметическими операциями.
...
При выполнении деления перед умножением в результате мы получили бы $-84,5$, остаток отброшен и результат был бы неверен. 
...
Теперь напишем пару простых тестов, которые покажут правильность выполнения программы. Запускать их будем вызывая саму программу, не используя отладку.
...
Так как адреса памяти все равно дают мало представления о том, как процедура выполняется для ```asm reverse``` в основном приведу только значения, хранящиеся в памяти до и после.
...
Адрес определяется в процессе копиляции, поэтому является константой. Формально для процессора мы перемещаем обычный длинный литерал. О том, что этот литерал -- адрес в оперативной памяти помнит только программист.
...
Где `100010` -- код ассемблерной команды ```asm mov```. `01` -- первое число записи означает направление, в данном случае из регистра, второе указывают на то, что операнды четырех байтовые. `11` -- указывает на то, что операнды - регистры. Далее идут номера самих регистров ```asm 000 = eax```, ```asm 001 = ecx```.
...
Где `100010` -- код ассемблерной команды ```asm mov```. `01` -- первое число записи означает направление, в данном случае из регистра, второе указывают на то, что операнды четырех байтовые. `00` -- указывает на то, что смещение в команде 0 байт, это и отличает эту команду от предыдущей, т.к. теперь мы интерпретируем принимающий регистр как адрес в оперативной памяти, в который мы записываем ```asm eax``` без смещения. Далее идут номера самих регистров ```asm 000 = eax```, ```asm 011 = ebx```.
